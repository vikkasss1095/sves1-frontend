...
export default function VendorDetails() {
  ...
  useEffect(() => {
    ...
    console.log("TOKEN:", token); // 🔥 CHECK 1

    api.get(`/admin/vendors/${id}`)
      .then(res => {
        console.log("RESPONSE:", res.data); // 🔥 CHECK 2
        setVendor(res.data);
      })
      .catch((err) => {
        console.log("ERROR:", err.response?.data || err.message); // 🔥 CHECK 3
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-4">
        <button
          onClick={() => api.put(`/admin/vendors/${id}/approve`).then(()=>toast.success("Approved"))}
        ...
        <button
          onClick={() => api.put(`/admin/vendors/${id}/reject`).then(()=>toast.success("Rejected"))}
